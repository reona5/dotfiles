// Vendored from @that-yolanda/pi-questionnaire v0.1.5 (MIT, author: that-yolanda)
//   https://github.com/that-yolanda/pi-extensions/tree/main/questionnaire
//
// なぜ fork しているか:
// 上流（および派生元である pi 同梱の examples/extensions/questionnaire.ts）は、
// ctx.ui.custom() に注入される KeybindingsManager を `_kb` として受け取って捨て、
// 選択肢の上下移動を Key.up / Key.down にハードコードしている。そのため
// keybindings.json の tui.select.up / tui.select.down（ここでは ctrl+p / ctrl+n）が
// 選択ダイアログに届かない。
//
// パッチ内容は "PATCH:" コメントを付けた2箇所だけ。差分を小さく保って上流に
// 追従しやすくしてある。上流に取り込まれたらこの vendor は捨てる。
// 同名の questionnaire ツールを提供するため、himorishige/pi-coding-agent-extensions
// 側の questionnaire.ts は settings.json のパッケージフィルタで除外している
// （plan-mode はツール名でしか参照していないので、そのまま動く）。
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const result = await ctx.ui.custom<QuestionnaireResult>(
				// PATCH: _kb → kb（注入される KeybindingsManager を使う）
				(tui, theme, kb, done) => {
					// State
					let currentTab = 0;
					let optionIndex = 0;
					...
					function handleInput(data: string) {
						...
						// Option navigation
						// PATCH: Key.up / Key.down 固定をやめ、tui.select.up /
						// tui.select.down の設定（ctrl+p / ctrl+n など）に従う。
