#!/bin/sh
# 会話で固まったドキュメント（仕様・設計資料など）を、現在ブランチに紐づく
# GitHub issue のコメントとして投稿する。
#
# 使い方: post-comment.sh <body-file>
#   <body-file> … 投稿する Markdown 本文のファイル。
#
# ブランチ名から issue 番号を推定し（grill 系 skill の resolve-issue.sh と同じ規則）、
# `gh issue comment` で投稿してコメント URL を標準出力に出す。
# 呼び出し側（skill）が AskUserQuestion で投稿可否を確認済みである前提。

body="$1"
[ -n "$body" ] || { echo "usage: post-comment.sh <body-file>" >&2; exit 2; }
